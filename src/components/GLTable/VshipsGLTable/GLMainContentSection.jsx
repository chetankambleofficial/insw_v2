...
const GLMainContentSection = ({
  ...
  return (
    <Box sx={{ position: "sticky", top: 0, bgcolor: "#fff", borderBottom: "1px solid #e5e7eb", px: 2.5, py: 2, zIndex: 10 }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 2 }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <Box sx={{ display: "flex", gap: 1.5 }}>
            {["load", "generate", "post"].map((step) => (
              <Box key={step} sx={{ textAlign: "center" }}>
                <Typography fontSize={10}>{workflowLabels[step]}</Typography>
                <LinearProgress
                  variant="determinate"
                  ...
                  sx={{
                    ...
                    borderRadius: 2,
                    bgcolor: "#e5e7eb",
                    "& .MuiLinearProgress-bar": { bgcolor: getProgressColor(step) },
                  }}
                />
              </Box>
            ))}
          ...
          {uploadSummary && (
            <Box sx={{ display: "flex", gap: 6, fontSize: 10, mt: 1, fontWeight: 500 }}>
              <span>Total: <b>{uploadSummary.totalCount}</b></span>
              <span>Success: <b style={{ color: "#2e7d32" }}>{uploadSummary.successCount}</b></span>
              <span>Errors: <b style={{ color: "#d32f2f" }}>{uploadSummary.errorCount}</b></span>
            </Box>
          )}
        </Box>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <TextField
            ...
            onChange={(e) => setFilters((p) => ({ ...p, effectiveDate: e.target.value }))}
          ...
          >
            {excelFile ? "Excel Selected" : "Upload Excel"}
