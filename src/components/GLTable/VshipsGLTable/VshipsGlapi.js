...
/* ================= LOAD WITH EXCEL ================= */
export const LoadVshipsGl = async ({
  year,
  month,
  effectiveDate,
  page = 0,
...
/* ================= GENERATE GL ================= */
export const postVshipsGl = async ({ year, month }) => {
  const formattedMonth = String(month).padStart(2, "0");
...
/* ================= VALIDATE GL ================= */
export const validateVshipsGl = async ({ periodName, page = 0, size = 15 }) => {
  const endpoint =
    `/v-ships/validate` +
...
export const getVshipsUploadSummary = async ({ periodName }) => {
  const endpoint =
    `/v-ships/upload-summary?periodName=${periodName}`;

  return doGet(endpoint);
};

/* ================= ERRORS ================= */
export const getVshipsErrors = async ({ periodName }) => {
  const endpoint =
    `/v-ships/errors?periodName=${periodName}`;
